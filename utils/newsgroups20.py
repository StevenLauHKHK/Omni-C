...
class Newsgroups20Dataset(Dataset):
    def __init__(self, root_dir, json_file, vocab_file, text_seq_len=128, augment=False, 
                 p_mask=0.15, p_delete=0.1, mask_token_id=103, pad_token_id=0, 
                 return_double_augmentations=False):
        """
        Newsgroups20 Dataset Loader for Contrastive Learning
        Args:
    ...
    def augment_text(self, tokens):
        """
        Apply light augmentations to a single token sequence.
        Args:
        ...
        for token in tokens:
            ...
            if prob < self.p_mask:
                ...
                continue  # Delete the token (skip adding to sequence)
            else:
                augmented_seq.append(token)  # Keep the token unchanged
    ...
    def __getitem__(self, idx):
        ...
        if self.return_double_augmentations:
            ...
            if self.augment:
                ...
                # Even without augment=True, apply minimal augmentations for contrastive learning
                tokens_1 = self.augment_text(tokens.copy())
