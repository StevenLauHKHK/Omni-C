...
class ImageDataset(Dataset):
    def __init__(self, root_dir, json_file, class_map_json=None, transform=None, return_double_augmentations=False, clip_align=False, clip_fine_tune=False, dataset=''):
        self.modality = 'image'
        ...
        self.clip_fine_tune = clip_fine_tune
        ...
        if self.clip_align:
            ...
            if self.dataset != '':
                self.text_templates = get_templates(self.dataset)
        # Load the class map
    ...
    def _load_class_map(self):
        ...
        with open(self.class_map_json, 'r') as f:
            ...
            if self.clip_fine_tune:
                # Convert to desired format
                class_map = {str(item["id"]): item["name"] for item in class_map}
